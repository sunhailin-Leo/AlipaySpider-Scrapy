...
class AlipaySpider(scrapy.Spider):
    ...
    def parse_personal(self, response):
        ...
        user_item['user_month_bill_info'] = \
            page_sel.xpath('string(//div[@class="amount-des"]/p[2]/span[@class="highlight"]/strong)').extract()[0]
        user_item['create_time'] = str(round(time.time() * 1000))
        yield user_item
        ...
        try:
            ...
            for tr in trs:
                ...
                try:
                    ...
                    try:
                        bill_info['transfer_name'] = tr.xpath('string(td[@class="name"]/p/text())').extract()[0].strip()
                    except IndexError:
